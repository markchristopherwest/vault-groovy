...
# Do AppRole config for Jenkins
echo "Preparing AppRole Auth Methods for Jenkins..."
cd ..
...
# Get the Role ID
vault read -format=json auth/approle/role/jenkins-role/role-id > sensitive/jenkins_vault_role_id.json
ROLE_ID=$(cat sensitive/jenkins_vault_role_id.json | jq -r .data.role_id)

# Get the Secret ID
vault write -format=json -f auth/approle/role/jenkins-role/secret-id > sensitive/jenkins_vault_secret_id.json
SECRET_ID=$(cat sensitive/jenkins_vault_secret_id.json | jq -r .data.secret_id)
